...
/// Build the batched head-probe query as a typed document.
///
/// Extracted from `graphql_batch` so it can be TESTED: the network call
/// cannot run under `cargo test`, but choosing the wrong selection can be
/// wrong without any network at all — which is exactly the defect this
/// replaced. Everything here is pure.
fn build_batch_query(targets: &[(usize, &UpstreamId)]) -> Result<String, anyhow::Error> {
    // Build the query as a TREE, not a string.
    //
    // This was `format!()` of GraphQL syntax, and it shipped a defect
    // that only a tree could have prevented — see theory/GRAPHQL-AST.md,
    // which is written around this exact site.
    //
    // ── ★ THERE IS NO SUCH REF AS refs/heads/HEAD ────────────────
    // `ExtendedNode::tracking_ref` returns the literal "HEAD" for any
    // input with no explicit ref — the ordinary `github:owner/repo`
    // form, which is most of the fleet. The old path fabricated
    // `refs/heads/HEAD`, GitHub resolved it to null rather than
    // erroring, and every such input came back with no head while the
    // REST fallback it was built to replace kept working.
    //
    // `defaultBranchRef` is a DIFFERENT SELECTION, not a different
    // string — which is why a string-building fix could not work, and
    // why the two cases below are visibly two branches building two
    // different `Field`s rather than two spellings of one format.
    use graphql_synthesizer::{
        aliased_batch, Field, GraphQlNode, Name, Operation, Selection, Value,
    };

    // Every name here is a literal from this file, so a failure is a
    // bug in this function rather than bad input — but it still goes
    // through `Name::new` rather than an unchecked constructor,
    // because there is no unchecked constructor.
    let n = |s: &str| -> Result<Name, anyhow::Error> {
        Name::new(s).map_err(|e| anyhow::anyhow!("invalid GraphQL name {s:?}: {e}"))
    };

    let commit_selection = || -> Result<Vec<Selection>, anyhow::Error> {
        Ok(vec![Selection::Field(
            Field::leaf(n("target")?).with_selections(vec![Selection::InlineFragment {
    ...
    for (_, id) in targets {
        ...
        let ref_field = if id.r#ref == "HEAD" || id.r#ref.is_empty() {
            // No explicit ref: ask for the default branch, which takes
            // NO argument at all. The old code could only express this
            // as a different string; here it is a different field.
            Field::leaf(n("defaultBranchRef")?).with_selections(commit_selection()?)
        } else {
            let qualified_ref = if id.r#ref.starts_with("refs/") {
                id.r#ref.clone()
            } else if looks_like_tag(&id.r#ref) {
                ["refs/tags/", &id.r#ref].concat()
            } else {
                ["refs/heads/", &id.r#ref].concat()
            };
            Field::leaf(n("ref")?)
                .with_arg(n("qualifiedName")?, Value::str(qualified_ref))
                .with_selections(commit_selection()?)
        };

        // `owner` and `repo` are VALUES beside the syntax, never text
        // inside it. `Value` has no variant that can hold source, so
        // the escaping question does not arise at this call site.
        fields.push(
            Field::leaf(n("repository")?)
                .with_arg(n("owner")?, Value::str(owner))
                .with_arg(n("name")?, Value::str(repo))
                .with_selections(vec![Selection::Field(ref_field)]),
        );
    }

    // `r0..rN` aliases, generated by the primitive rather than by a
    // format string — the one piece of this query that is genuinely
    // structural rather than an operand.
    Ok(GraphQlNode::Operation(Operation::query().with_selections(aliased_batch(fields))).emit(0))
}
...
#[cfg(test)]
mod tests {
    // ── the GraphQL batch, built as a tree ───────────────────────────
    //
    // These assert on the EMITTED QUERY. The old code was a format string
    // and the defect it shipped was a wrong SELECTION, which no amount of
    // asserting on inputs would have caught.

    fn uid(key: &str, r: &str) -> UpstreamId {
        UpstreamId {
    ...
    #[test]
    fn no_explicit_ref_asks_for_defaultBranchRef_with_no_argument() {
        // THE regression. "HEAD" used to become `refs/heads/HEAD`, a ref
        // that does not exist; GitHub answered null instead of erroring and
        // the default path silently returned nothing for most of the fleet.
        let id = uid("pleme-io/escriba", "HEAD");
        let q = super::build_batch_query(&[(0, &id)]).expect("build");
        assert!(q.contains("defaultBranchRef"), "{q}");
        assert!(
            !q.contains("refs/heads/HEAD"),
            "the fabricated ref must be gone: {q}"
        );
        assert!(
            !q.contains("qualifiedName"),
            "the default-branch form takes no ref argument at all: {q}"
        );
    }

    #[test]
    fn an_empty_ref_is_treated_the_same_as_HEAD() {
        let id = uid("pleme-io/escriba", "");
        let q = super::build_batch_query(&[(0, &id)]).expect("build");
        assert!(q.contains("defaultBranchRef"), "{q}");
    }

    #[test]
    fn an_explicit_branch_qualifies_and_uses_ref() {
        let id = uid("pleme-io/escriba", "main");
        let q = super::build_batch_query(&[(0, &id)]).expect("build");
        assert!(
            q.contains(r#"ref(qualifiedName: "refs/heads/main")"#),
            "{q}"
        );
        assert!(!q.contains("defaultBranchRef"), "{q}");
    }
    ...
    #[test]
    fn targets_are_aliased_r0_upward() {
        let a = uid("pleme-io/escriba", "HEAD");
        let b = uid("pleme-io/tend", "main");
        let q = super::build_batch_query(&[(0, &a), (1, &b)]).expect("build");
        assert!(
            q.contains(r#"r0: repository(owner: "pleme-io", name: "escriba")"#),
            "{q}"
        );
        assert!(
            q.contains(r#"r1: repository(owner: "pleme-io", name: "tend")"#),
            "{q}"
        );
    }

    #[test]
    fn an_operand_cannot_become_syntax() {
        // The shape `format!("owner: \"{owner}\"")` permitted and a typed
        // Value does not: a key that closes the literal and continues as
        // query source. It is escaped, so the document keeps ONE repository
        // field rather than growing a second selection.
        let id = uid(r#"a") { viewer { email } } x(y: "/b"#, "HEAD");
        let q = super::build_batch_query(&[(0, &id)]).expect("build");
        assert_eq!(
            q.matches("repository(").count(),
            1,
            "a hostile operand added structure: {q}"
        );
        assert!(q.contains(r#"\""#), "quotes are escaped: {q}");
    }
